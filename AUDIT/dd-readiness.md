...
- **Concrete evidence the base branch does *not* cleanly restore:** the PR CI for this audit surfaced `.NET Security Scan (src/services/benefit-plan-service)` **failing on `main`** with `error NU1605` — `benefit-plan-service.csproj` directly pinned `MongoDB.Driver` 3.10.0 while its dependency `CloudHealthOffice.BenefitEngine` was bumped to 3.11.0 in Dependabot PR #1101, and the warnings-as-errors build turns the transitive downgrade into a hard restore failure. This was a live break on `main` affecting every open PR (fixed separately in PR #1105 by aligning to 3.11.0). Two takeaways for DD: (a) grouped Dependabot bumps are landing without a full cross-project restore gate, and (b) **`MongoDB.Driver` versions are sprawled across the repo** (3.6.0 ×27, 3.7.1 ×4, 2.28.0 ×1, 3.11.0 ×4) — consolidate to one version (ideally via `Directory.Packages.props` central package management) to prevent recurrence.
