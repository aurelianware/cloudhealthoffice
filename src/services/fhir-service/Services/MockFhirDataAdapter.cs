...
public class MockFhirDataAdapter : IFhirDataAdapter
{
    ...
    private static readonly List<ExplanationOfBenefit> Eobs =
    [
        new()
        {
            Id = "eob-001",
            ...
            Insurance = [new ExplanationOfBenefit.InsuranceComponent { Focal = true, Coverage = new ResourceReference("Coverage/cov-001") }],
            Item =
            [
                new()
                {
        ...
        new()
        {
            ...
            Provider = new ResourceReference("Practitioner/prov-001"),
            Outcome = ClaimProcessingCodes.Complete,
            Insurance = [new ExplanationOfBenefit.InsuranceComponent { Focal = true, Coverage = new ResourceReference("Coverage/cov-002") }],
            Item =
            [
                new()
                {
    ...
    public Task<ExplanationOfBenefit?> GetEobAsync(string id, string tenantId, CancellationToken ct = default)
        => Task.FromResult(Eobs.FirstOrDefault(e => e.Id == id));

    public Task<(IReadOnlyList<ExplanationOfBenefit> Items, int Total)> SearchEobsAsync(
        EobSearchParams search, string tenantId, CancellationToken ct = default)
    {
        var query = Eobs.AsEnumerable();
        ...
        {
            var ref_ = NormalizeRef("Patient", search.Patient);
            query = query.Where(e => e.Patient?.Reference == ref_);
        }

        if (!string.IsNullOrEmpty(search.Status) &&
            Enum.TryParse<ExplanationOfBenefit.ExplanationOfBenefitStatus>(search.Status, true, out var status))
            query = query.Where(e => e.Status == status);

        var all = query.ToList();
        var page = all.Skip((search.Page - 1) * search.Count).Take(search.Count).ToList();

        return Task.FromResult<(IReadOnlyList<ExplanationOfBenefit>, int)>((page, all.Count));
    }
