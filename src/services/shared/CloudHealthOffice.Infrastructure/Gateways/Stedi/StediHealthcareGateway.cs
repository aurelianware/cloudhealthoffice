...
public sealed class StediHealthcareGateway : IEligibilityGateway, IClaimSubmissionGateway, IClaimAcknowledgmentGateway, IClaimAttachmentGateway, IClaimStatusGateway, IRemittanceGateway
{
    ...
    public async Task<GatewayResponse<ClaimStatusResponse>> CheckClaimStatusAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (configErrors.Count > 0)
        {
            return StatusConfigFail(request, startedAt, stopwatch,
                "Stedi gateway is not configured correctly: " + string.Join(" ", configErrors));
        }

        if (_statusClient is null)
        {
            return StatusConfigFail(request, startedAt, stopwatch,
                "Stedi claim status client is not registered.");
        }

        return await _claimStatus.InquireAsync(
            GatewayName,
            request,
            async (filled, ct) =>
            {
                var resolution = await _payerResolver
                    .ResolveAsync(filled.TenantId, filled.PayerId, HealthcareTransactionType.ClaimStatus276277, ct)
                    .ConfigureAwait(false);
                if (resolution.Status != PayerResolutionStatus.Found ||
                    string.IsNullOrWhiteSpace(resolution.ExternalIdentifierValue))
                {
                    return new ClaimStatusTransportResult(
                        false, null, 0, MapResolution(resolution.Status),
                        resolution.Message ?? "Payer could not be resolved for this claim status inquiry.",
                        null);
                }

                try
                {
                    var stediRequest = StediClaimStatusMapper.ToStediRequest(
                        filled, resolution.ExternalIdentifierValue);
                    var apiResult = await _statusClient.CheckAsync(stediRequest, ct).ConfigureAwait(false);
                    var canonical = StediClaimStatusMapper.ToCanonical(apiResult.Response, filled);
                    canonical.ExternalTransactionId ??= apiResult.ExternalTransactionId;
                    var category = StediClaimStatusMapper.BusinessCategory(canonical);
                    return new ClaimStatusTransportResult(
                        true, canonical, apiResult.RetryCount, category, null, apiResult.ExternalTransactionId);
                }
                catch (StediApiException ex)
                {
                    return new ClaimStatusTransportResult(
                        false, null, ex.RetryCount, ex.Category, ex.Message, null);
                }
            },
            cancellationToken).ConfigureAwait(false);
    }

    private GatewayResponse<ClaimStatusResponse> StatusConfigFail(
        ClaimStatusRequest request,
