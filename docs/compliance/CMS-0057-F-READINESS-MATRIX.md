...
| **Integration required** | CHO has the service surface or projection pattern, but production use depends on wiring payer source systems, live adapters, tenant config, and operational controls. | "Available as part of implementation." |
...
| **Out of CHO scope** | Requirement typically depends on a payer, PBM, HIE, provider, or state system outside CHO ownership. | "Coordinate through payer ecosystem integration." |
...
that deploys beside a payer's existing CAPS and turns CHO into the FHIR,
...
4. **Prior-authorization metrics reporting needs a product artifact.** CMS
   reporting can become a differentiator if CHO generates the public report and
