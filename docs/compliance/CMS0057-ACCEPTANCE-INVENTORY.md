...
| `Services/Clinical/ClinicalResourceInventory.cs` | the 12 USCDI clinical types + subject element + implemented search params — read by the routes, SMART, the Provider Access filter, the P2P import policy and the CapabilityStatement | n/a (pure) | PAT-02 |
| `Controllers/ClinicalResourceController.cs` | `GET {ClinicalType}/{id}` and `GET {ClinicalType}?patient=|subject=|_id=`, routes constrained to the inventory | Demo/Live (follows the store) | PAT-02 |
| `Services/Clinical/ClinicalResourceService.cs` | member-keyed read/search + uniform refusals + PHI-free audit | Demo/Live | PAT-02 |
| `Services/Clinical/ClinicalResourceStore.cs` | `IClinicalResourceStore` — served by the P2P import repositories (one store, two interfaces) | Mongo when configured, in-process otherwise | PAT-02 |
| `Services/Clinical/ClinicalResourceProjector.cs` | stored row → served resource: CHO id, trusted member binding, reference resolution, `meta.source`/`versionId` | n/a (pure) | PAT-02 |
| `Services/Clinical/ClinicalPayloadValidator.cs` | type / source-id / size / nesting gate before clinical data becomes readable PHI | config (`Clinical:PayloadLimits`) | PAT-02 |
| `Services/Clinical/ClinicalBackfill.cs` | re-projects clinical data from exchanges committed before PAT-02; deterministic, replay-safe, non-destructive, **disabled by default** | config (`Clinical:Backfill`) | PAT-02 migration |
| `Services/IPatientAccessDataProvider.cs` | `MockPatientAccessDataProvider` (synthetic pat-001..003) | Demo | PROV-01/02, PAT-01 |
| `Controllers/SmartConfigurationController.cs` | `.well-known/smart-configuration`, advertising the configured issuer's discovered endpoints | Demo or ExternalIssuer | SEC-01 |
| `Services/Identity/SmartTrustOptions.cs` | Demo/ExternalIssuer mode, fail-closed configuration validation | Both | SEC-01 |
| `Services/Identity/TrustedIssuerRegistry.cs` | exact-issuer trust resolution, per-issuer policy | Both | SEC-01 |
| `Services/Identity/SmartSigningKeyRing.cs` | JWKS cache, rotation on unknown kid, single-flight refresh, outage bounds | Both | SEC-01 |
| `Services/Identity/HttpIssuerMetadataFetcher.cs` | OIDC discovery validation, JWKS ingestion, SSRF origin policy | Both | SEC-01 |
| `Services/Identity/CallerIdentityResolver.cs` | caller identity, provider NPI from a trusted claim only | Both | SEC-01 |
| `Services/Identity/SmartIdentityTrustHealthCheck.cs` | identity trust readiness | Both | SEC-01 |
...
| PAT-02 US Core clinical | **PASSABLE** | n/a | Demographics validate against US Core, and the **USCDI clinical set is now served by CHO's own FHIR surface**: `AllergyIntolerance`, `CarePlan`, `CareTeam`, `Condition`, `Device`, `DiagnosticReport`, `Goal`, `Immunization`, `MedicationDispense`, `MedicationRequest`, `Observation`, `Procedure`. One table (`ClinicalResourceInventory`) is read by the routes, the SMART layer, the Provider Access filter, the P2P import classification and the CapabilityStatement, with structural tests pinning each to it, so a type cannot be reachable through one layer and forgotten by another. The Payer-to-Payer import store was **promoted** into the clinical serving store — one object behind two interfaces, no projection, no dual write — so clinical data stays out of CHO's authoritative member/coverage/claim stores by construction and reads see committed exchange state only. Identity is the deterministic import key (tenant + member + source payer + type + source id), so replay updates in place, two payers' identically-numbered records never merge, and another tenant's id resolves to nothing. Member binding comes from the trusted exchange context and the served subject is rewritten to it, so an imported `subject` is never authorization authority. Patient Access enforces the token binding against **both** `patient` and `subject`; Provider Access keeps all four CONSENT-01 controls because the clinical types are in the same global filter's governed set. Every store call is keyed on the authorized member, so a direct id read cannot reach another member's data, and not-yours/not-there are one uniform 404. Clinical payloads pass a size/nesting/source-id gate; refusals and still-unsupported types are named and counted, never dropped. A deterministic, replay-safe, non-destructive backfill makes clinical data from **earlier** exchanges readable without re-running them. `meta.source`/`meta.versionId` keep imported data distinguishable from CHO-authored. **No US Core profile is claimed** — valid R4, not validated conformance — and search is `_id`/`patient`/`subject` only. Full design: [docs/architecture/clinical-fhir.md](../architecture/clinical-fhir.md) |
...
| SEC-01 SMART/OAuth | **PASSABLE** | n/a | multi-issuer trust registry, validated OIDC discovery + JWKS rotation, per-issuer audience/algorithm/claim policy, caller identity, fail-closed production startup; no customer IdP connected in-repo |
...
**What changed in the SEC-01 PR:** CHO became a production-configurable SMART/
OAuth resource server — an explicit registry of trusted external OIDC issuers,
validated discovery and JWKS retrieval, key rotation, per-issuer audience and
algorithm policy, a caller identity model, and fail-closed production startup
that refuses to run Demo trust. SEC-01 moved PARTIAL → **PASSABLE**. Full design:
[docs/architecture/smart-oauth-trust.md](../architecture/smart-oauth-trust.md)
and the [IdP integration contract](../architecture/idp-integration-contract.md).

**This does not mean an identity provider has been connected.** Every issuer in
the test suite is synthetic. What the product owns — trust configuration,
validation, rotation, identity mapping, isolation — is implemented and tested;
what an engagement owns is pointing it at that customer's IdP and running the
validation checklist in the integration contract.

**What changed in the PAT-02 PR:** the USCDI clinical resource types are served
through Patient and Provider Access from durable, tenant- and member-isolated
storage, instead of being archived as unsupported. PAT-02 moved PARTIAL →
**PASSABLE**. Full design:
[docs/architecture/clinical-fhir.md](../architecture/clinical-fhir.md).

The shape of it: **one inventory table** feeds the routes, the SMART scope
layer, the Provider Access consent filter, the Payer-to-Payer import
classification and the CapabilityStatement, so a clinical type cannot be served
without also being governed. The Payer-to-Payer import store was **promoted**
into the clinical serving store rather than projected into a second one — the
rows a reader gets are the rows an exchange committed, which is why there is
nothing to fall behind and nothing to reconcile. Reads are keyed on the
authorized member inside the storage query, so another member's resource is
never selected rather than filtered out afterwards. A **backfill** re-projects
clinical data from exchanges committed before this feature, so no operator has
to ask a prior payer again for data CHO already holds.

Stated limitations, not glossed: no US Core profile is claimed or validated
(valid R4 only); search is `_id`, `patient` and `subject` where R4 defines it —
no `category`, `code`, `status` or date search; and no CHO component authors
native clinical data yet, so every served clinical resource is imported.
...
at an A4 status. PAS-07 moved PARTIAL → **PASSABLE**. Full design:
