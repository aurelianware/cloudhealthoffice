...
**PASSABLE**, so CHO Replace declares **20 PASSABLE / 1 PARTIAL / 0 GAP**
(generator-computed from the manifest — nothing here is hard-coded). The one
remaining PARTIAL is **SEC-01**, where the identity provider is configured per
engagement. Full design:
