...
if [ "$SFTP_IP" = "pending" ] || [ -z "$SFTP_IP" ]; then
    ...
    echo ""
    echo "📋 Accounts:"
    echo "   Username: cho-edi          Directory: /home/cho-edi/upload"
    echo "   Username: clearinghouse    Directory: /home/clearinghouse/edi"
fi

echo ""
echo "⚠️  The manifest ships placeholder passwords (REPLACE_WITH_SFTP_PASSWORD)."
echo "   The server will not accept logins until you set real ones:"
echo ""
echo "   kubectl create secret generic sftp-users -n cho-sftp \\"
echo "     --from-literal=users.conf=\"\$(printf '%s\\n%s\\n' \\"
echo "       \"cho-edi:<password>:1000:1000:upload\" \\"
echo "       \"clearinghouse:<password>:1001:1001:edi\")\" \\"
echo "     --dry-run=client -o yaml | kubectl apply -f -"
echo ""
echo "   Then give the X12 jobs the same password:"
echo "   kubectl create secret generic sftp-credentials -n cho-workflows \\"
